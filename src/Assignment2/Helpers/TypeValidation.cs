...
namespace Assignment2.Helpers
{
    ...
    {
        /// <summary>
        /// Checks whether the input is char or not
        /// </summary>
        /// <param name="option">Input to be checked</param>
        /// <param name="optionChar">Stores char value of input if true,else false</param>
        /// <returns>True if parsing is successfull, else False</returns>
        public static bool IsValidChar(string option, out char optionChar)
        {
            if (char.TryParse(option, out optionChar))
            {
                return true;
            }
            else
